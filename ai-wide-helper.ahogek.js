...
(function () {
  ...
  // ============================================================
  // 第二部分：必须等待 Body 加载的 UI 逻辑
  // ============================================================
  function initUI() {
    console.log('[AI Widescreen] Body ready, initializing UI...');
    ...
    if (host.includes('perplexity.ai')) {
      ...
      const updateGeminiStyles = () => {
        if (!document.getElementById('ai-widescreen-style')) document.head.appendChild(styleElement);
        if (isGeminiChat()) {
          if (styleElement.textContent !== geminiCSS) styleElement.textContent = geminiCSS;
        } else {
  ...
  // ============================================================
  // 第三部分：智能 Bootloader (确保 Body 存在后再执行 UI 逻辑)
  // ============================================================
  if (document.body) {
    initUI();
  } else {
    ...
    // 双重保险：有时候 DOMContentLoaded 也会错过 body，观察 html 根节点
